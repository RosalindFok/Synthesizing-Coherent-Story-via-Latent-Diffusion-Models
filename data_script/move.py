...
dirs_path_list = [x for x in dirs_path_list if any(y in x for y in ['3-', 'test']+valid_dirs)]
...
for dir in tqdm(dirs_path_list, desc='Moving', leave=True):
    ...
    for image in image_path:
        ...
        if '.json' in image:
            ...
            caption[image_name] = data[image_name].replace('-', '').replace('/', '').replace('`', '').replace('|', '').encode('utf-8').decode('unicode_escape')
